      ...
      <main className="container mx-auto py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {events.map(event => (
            <div
              key={event.id}
              className="cursor-pointer rounded-lg overflow-hidden shadow-md transition-all duration-300 transform hover:scale-105 hover:shadow-md"
            >
              <img
                src={`http://localhost:8000/${event.image}`}
                className="w-full h-64 object-cover object-center rounded-t-lg hover:opacity-75 transition duration-300"
              ...
              <div className="p-6 bg-white rounded-b-lg">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <button
                    className={`px-4 py-2 ${event.is_liked ? 'bg-red-500' : 'bg-blue-500'} text-white rounded-full hover:bg-opacity-75 transition duration-300`}
